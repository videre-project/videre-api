      ...
      if (page?.has_more == true) {
        ...
        for (let i = 2; i <= numPages; i++) {
          ...
          const _page = await fetch(
            `https://api.scryfall.com/cards${tag.uri}&page=${i}`
          ).then(res => res.json());
...
export default async (req, res) => {
  ...
  if (source.includes('scryfall')) {
    const tags = [...new Set(await getScryfallTags({ functional: true }))].filter(
      tag => tag.name == 'burn' || tag.name == 'removal'
    );
