...
import { getParams, eventsQuery } from 'utils/querybuilder.js';
...
export default async (req, res) => {
    ...
    const _format = getParams(params, 'f', 'format')
        .map(obj => {
    ...
    const _type = getParams(params, 't', 'type')
        .map(obj => {
    ...
    const archetypes = request_2.map(obj => {
        ...
        return {
            ...
            deck: [
                ...obj.deck?.mainboard.map(_obj => ({
                    ..._obj, container: 'mainboard'
                })),
                ...obj.deck?.sideboard.map(_obj => ({
                    ..._obj, container: 'sideboard'
                }))
            ],
            deck_uid: obj.uid,
        }
    ...
    const cards = archetypes.map(obj => {
            ...
            "archetypes": {
                ...
                "data": archetypes.filter((obj, i) =>
                    ...
                    ).map(obj => (
                        {
                            ...
                            "percentage": (archetypes.filter((_obj) => _obj.uid === obj.uid).length
            ...
            "cards": {
                ...
                "data": cards.filter((obj, i) =>
                    ...
                    ).map(obj => (
                        {
                            ...
                            "percentage": parseFloat(([...new Set(
                                cards.filter((_obj) => _obj.cardname === obj.cardname)
                                    .map(_obj => _obj.deck_uid)
                                )].length / [...new Set(cards.map(_obj => _obj.deck_uid))].length
                                * 100)).toFixed(2) + '%',
                            "average": parseFloat((
                                cards.filter((_obj) => _obj.cardname === obj.cardname)
